...
def setup_base_data(conn):
    ...
    # ADICIONE ESTAS LINHAS AQUI - Criar o brand principal PRIMEIRO
    cursor.execute("""
        INSERT INTO brands (id, name, created_at) 
        VALUES (%s, %s, %s) 
        ON CONFLICT (id) DO NOTHING
    """, (BRAND_ID, 'Challenge Brand', datetime.now()))
    conn.commit()  # Commit importante para garantir que o brand existe
    
    # Sub-brands (agora vai funcionar)
